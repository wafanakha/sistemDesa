...
const CreateKeteranganLetter: React.FC = () => {
  ...
  const handleExportPDF = () => {
    ...
    const data = [
      ['1. Nama', form.nama],
      ['2. Tempat/Tgl Lahir', `${form.tempatLahir}, ${form.tanggalLahir && new Date(form.tanggalLahir).toLocaleDateString('id-ID')}`],
      ...
      ['9. Berlaku', form.berlakuDari && form.berlakuSampai ? `${new Date(form.berlakuDari).toLocaleDateString('id-ID')} s/d ${new Date(form.berlakuSampai).toLocaleDateString('id-ID')}` : ''],
    ...
    y += 2;
    doc.text('Demikian Surat Keterangan ini diberikan untuk dipergunakan seperlunya.', 15, y, { maxWidth: pageWidth - 30 });
    y += 18;
    // TTD
    // Pemohon kiri, pejabat kanan
    doc.setFontSize(10);
    doc.text('Pemohon', 30, y);
    doc.text(`Kedungwringin, ${new Date().toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}`, pageWidth - 15, y, { align: 'right' });
    y += 6;
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-4 text-center text-teal-800">Surat Keterangan</h1>
      <div className="mb-4">
        <input
          type="text"
      ...
      <form className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <input name="warganegara" value={form.warganegara} onChange={handleChange} placeholder="Warganegara" className="input" />
        ...
        <div className="flex gap-2">
          <div className="flex flex-col flex-1">
            ...
            <input name="berlakuDari" value={form.berlakuDari} onChange={handleChange} type="date" className="input" />
          </div>
          <div className="flex flex-col flex-1">
            <label className="text-xs text-gray-600 mb-1">Berlaku sampai</label>
            <input name="berlakuSampai" value={form.berlakuSampai} onChange={handleChange} type="date" className="input" />
          </div>
        </div>
      ...
      <div id="keterangan-preview" className="bg-white p-8 border shadow max-w-[800px] mx-auto">
        ...
        <div className="content mt-4">
          <p style={{textIndent: '2em'}}>Yang bertanda tangan di bawah ini, kami Kepala Desa Kedungwringin Kecamatan Patikreja Kabupaten Banyumas Provinsi Jawa Tengah, menerangkan bahwa:</p>
          <table className="mb-2">
            <tbody>
              ...
              <tr><td>3. Warganegara</td><td>:</td><td>{form.warganegara}</td></tr>
              ...
              <tr><td>8. Keperluan</td><td>:</td><td>{form.keperluan}</td></tr>
              <tr><td>9. Berlaku</td><td>:</td><td>{form.berlakuDari && form.berlakuSampai ? `${new Date(form.berlakuDari).toLocaleDateString('id-ID')} s/d ${new Date(form.berlakuSampai).toLocaleDateString('id-ID')}` : ''}</td></tr>
              <tr><td>10. Keterangan lain</td><td>:</td><td>{form.keteranganLain}</td></tr>
            </tbody>
          </table>
          <p style={{textIndent: '2em'}}>Demikian Surat Keterangan ini diberikan untuk dipergunakan seperlunya.</p>
        </div>
