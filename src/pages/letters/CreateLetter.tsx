...
const letterTypes = [
  ...
  },
  {
    ...
    description: 'Surat pernyataan belum menikah untuk keperluan administrasi atau persyaratan nikah.',
    path: '/letters/create/belum-menikah',
  },
  {
    type: 'kematian',
    label: 'Surat Keterangan Kematian (Model N6)',
    description: 'Surat keterangan kematian sesuai format N6, untuk keperluan administrasi kematian.',
    path: '/letters/create/kematian',
  },
  {
    type: 'pengantar-nikah',
    label: 'Surat Pengantar Nikah (N1)',
    description: 'Formulir pengantar nikah Model N1, lengkap dengan pencarian KK dan pemilihan ibu.',
    path: '/letters/create/pengantar-nikah',
  },
  {
    type: 'permohonan-kehendak-nikah',
    label: 'Permohonan Kehendak Nikah (N2)',
    description: 'Surat permohonan kehendak nikah (N2), diisi manual oleh pemohon.',
    path: '/letters/create/permohonan-kehendak-nikah',
  },
  {
    type: 'persetujuan-calon-pengantin',
    label: 'Persetujuan Calon Pengantin (N4)',
    description: 'Surat persetujuan calon pengantin (N4), diisi manual oleh pemohon.',
...
const CreateLetter: React.FC = () => {
  ...
  const suratBiasa = letterTypes.filter(
    (s) => s.type !== 'wali-nikah' && s.type !== 'pengantar-numpang-nikah' && s.type !== 'belum-menikah'
  );
  // Daftar surat nikah
  const suratNikah = letterTypes.filter(
    (s) => s.type === 'wali-nikah' || s.type === 'pengantar-numpang-nikah' || s.type === 'belum-menikah' || s.type === 'pengantar-nikah'
  );
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 justify-start items-stretch">
        {suratBiasa.map((item) => (
          <div key={item.type} className="bg-white border rounded shadow p-4 flex flex-col cursor-pointer hover:bg-teal-50" onClick={() => navigate(item.path)}>
            <h2 className="font-semibold text-lg text-teal-800 mb-1">{item.label}</h2>
      ...
      {suratNikah.length > 0 && (
        <>
          <h2 className="text-xl font-bold mt-10 mb-4 text-left text-teal-700">Surat Perihal Nikah</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 justify-start items-stretch">
            {suratNikah.map((item) => (
              <div key={item.type} className="bg-white border rounded shadow p-4 flex flex-col cursor-pointer hover:bg-teal-50" onClick={() => navigate(item.path)}>
                <h2 className="font-semibold text-lg text-teal-800 mb-1">{item.label}</h2>
