...
const CreateLetter: React.FC = () => {
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        const defaultTemplate = await letterService.getDefaultTemplateByType(
          selectedLetterType
        );
  ...
  const populateLetterContent = () => {
    ...
    const birthDate = new Date(selectedResident.birthDate).toLocaleDateString(
      "id-ID",
      {
        day: "2-digit",
        month: "long",
    ...
    content = content.replace(
      /\[RESIDENT_OCCUPATION\]/g,
      selectedResident.occupation
    );
    content = content.replace(
      /\[RESIDENT_MARITAL_STATUS\]/g,
    ...
    content = content.replace(
      /\[VILLAGE_LEADER_NAME\]/g,
      villageInfo.leaderName
    );
    content = content.replace(
      /\[VILLAGE_LEADER_TITLE\]/g,
      villageInfo.leaderTitle
    );
  ...
  const onSubmit = async (data: LetterFormData) => {
    ...
    try {
      ...
      const letterNumber = await letterService.generateLetterNumber(
        data.letterType
      );
