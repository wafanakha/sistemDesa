...
const CreateDomisiliUsahaLetter: React.FC<{ editData?: Letter; isEditMode?: boolean }> = ({ editData, isEditMode }) => {
  ...
  const handleExportPDF = async () => {
    const preview = document.getElementById('domisili-usaha-preview');
    if (!preview) return;
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-4 text-center text-teal-800">Surat Keterangan Domisili Usaha</h1>
      <div className="mb-4">
        <input
          type="text"
      ...
      <form className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <input name="jumlahKaryawan" value={form.jumlahKaryawan || ''} onChange={handleChange} placeholder="Jumlah Karyawan" className="input" />
        <input name="luasTempatUsaha" value={form.luasTempatUsaha || ''} onChange={handleChange} placeholder="Luas Tempat Usaha" className="input" />
        <input name="waktuUsaha" value={form.waktuUsaha || ''} onChange={handleChange} placeholder="Waktu Usaha" className="input" />
      </form>
      ...
      <div id="domisili-usaha-preview" className="bg-white p-8 border shadow max-w-[800px] mx-auto">
        <div className="container" style={{ width: '210mm', minHeight: '297mm', padding: '1cm', margin: 'auto', background: 'white', boxSizing: 'border-box' }}>
          ...
          <h2 style={{ textAlign: 'center', textDecoration: 'underline', fontSize: '11pt' }}>SURAT KETERANGAN DOMISILI USAHA</h2>
          ...
          <div className="content" style={{ marginTop: 30 }}>
            ...
            <table style={{ marginLeft: 20 }}>
              <tbody>
                ...
                <tr><td>7. Alamat</td><td>:</td><td>{form.alamat}</td></tr>
              </tbody>
            </table>
            <p>Berdasarkan Surat Pernyataan tidak keberatan/ijin tetangga yang diketahui Ketua Rukun Tetangga dan Ketua Rukun Warga Nomer Tanggal, bahwa yang bersangkutan benar telah membuka Usaha sebagai berikut:</p>
            <table style={{ marginLeft: 20 }}>
              <tbody>
                <tr><td>Nama Perusahaan</td><td>:</td><td>{form.namaUsaha}</td></tr>
                <tr><td>Nama Pemilik</td><td>:</td><td>{form.nama}</td></tr>
                <tr><td>Alamat Perusahaan</td><td>:</td><td>{form.alamatUsaha}</td></tr>
                <tr><td>Jenis Usaha</td><td>:</td><td>{form.jenisUsaha}</td></tr>
                <tr><td>Jumlah Karyawan</td><td>:</td><td>{form.jumlahKaryawan}</td></tr>
                <tr><td>Luas Tempat Usaha</td><td>:</td><td>{form.luasTempatUsaha}</td></tr>
                <tr><td>Waktu Usaha</td><td>:</td><td>{form.waktuUsaha}</td></tr>
              </tbody>
            </table>
            <p>Demikian Surat Keterangan Domisili Usaha ini dibuat untuk keperluan {form.keperluan || 'mengajukan Permohonan Surat Ijin Tempat Usaha/Ijin Undang-undang Gangguan dari Pemerintah Kabupaten Banyumas. Surat ini berlaku 3 (tiga) bulan setelah dikeluarkan, bukan merupakan surat ijin, dan tidak diperkenankan untuk melakukan usaha sebelum mendapat ijin resmi dari instansi terkait'}.</p>
          </div>
          <div className="footer-info" style={{ display: 'flex', justifyContent: 'left', marginTop: 20, marginBottom: 10 }}>
            <table>
              <tbody>
                <tr>
          ...
          <div className="signature-block" style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20 }}>
            <div className="signature" style={{ width: '30%', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', minHeight: 60 }}>
              <div>
                <p>Mengetahui,</p>
                <p>Camat Patikreja</p>
              </div>
              <div style={{ marginTop: 'auto' }}>
                <div className="ttd-space" style={{ minHeight: 60, borderBottom: '1px solid transparent' }}></div>
