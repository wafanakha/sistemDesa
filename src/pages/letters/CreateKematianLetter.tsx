...
function generateSuratKematianN6Pdf(form: any) {
  ...
  // Section A
  let y = 62;
  doc.text(`A. 1. Nama lengkap dan alias\t: ${form.deceasedName}${form.deceasedAlias ? " / " + form.deceasedAlias : ""}`, 10, y);
  doc.text(`2. Bin/Binti\t\t\t\t: ${form.deceasedBinBinti}`, 10, y + 6);
  doc.text(`3. Nomor Induk Kependudukan\t: ${form.deceasedNik}`, 10, y + 12);
  doc.text(`4. Tempat dan tanggal lahir\t\t: ${form.deceasedBirthPlace}, ${form.deceasedBirthDate}`, 10, y + 18);
  doc.text(`5. Kewarganegaraan\t\t\t\t: ${form.deceasedNationality}`, 10, y + 24);
  ...
  doc.setFont("helvetica", "bold");
  doc.text(`Telah meninggal dunia pada tanggal\t: ${form.deathDate}`, 10, y + 50);
  doc.text(`Di\t\t\t\t\t\t: ${form.deathPlace}`, 10, y + 56);
  // Section B
  doc.setFont("helvetica", "normal");
  doc.text("Yang bersangkutan adalah suami/istri dari :", 10, y + 66);
  let yB = y + 74;
  doc.text(`B. 1. Nama lengkap dan alias\t\t: ${form.spouseName}${form.spouseAlias ? " / " + form.spouseAlias : ""}`, 10, yB);
  doc.text(`2. Bin/Binti\t\t\t\t: ${form.spouseBinBinti}`, 10, yB + 6);
  doc.text(`3. Nomor Induk Kependudukan\t: ${form.spouseNik}`, 10, yB + 12);
  doc.text(`4. Tempat dan tanggal lahir\t\t: ${form.spouseBirthPlace}, ${form.spouseBirthDate}`, 10, yB + 18);
  doc.text(`5. Kewarganegaraan\t\t\t\t: ${form.spouseNationality}`, 10, yB + 24);
  ...
  doc.text(
    `Kedungwiringin, ${form.suratDate && new Date(form.suratDate).toLocaleDateString("id-ID")}`,
    120,
    yB + 66
  );
...
const CreateKematianLetter: React.FC = () => {
  ...
  const handleExportPdf = () => {
    generateSuratKematianN6Pdf(form);
  };

  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold">Surat Keterangan Kematian (Model N6)</h2>
      <form className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-6 rounded shadow mb-4">
        <div className="md:col-span-2 font-semibold mb-2">A. Data Almarhum/Almarhumah</div>
        <Input label="Nama Lengkap" name="deceasedName" value={form.deceasedName} onChange={handleChange} required />
        <Input label="Alias" name="deceasedAlias" value={form.deceasedAlias} onChange={handleChange} />
        <Input label="Bin/Binti" name="deceasedBinBinti" value={form.deceasedBinBinti} onChange={handleChange} />
        <Input label="NIK" name="deceasedNik" value={form.deceasedNik} onChange={handleChange} />
        <Input label="Tempat Lahir" name="deceasedBirthPlace" value={form.deceasedBirthPlace} onChange={handleChange} />
        <Input label="Tanggal Lahir" name="deceasedBirthDate" type="date" value={form.deceasedBirthDate} onChange={handleChange} />
        <Input label="Kewarganegaraan" name="deceasedNationality" value={form.deceasedNationality} onChange={handleChange} />
        <Input label="Agama" name="deceasedReligion" value={form.deceasedReligion} onChange={handleChange} />
        <Input label="Pekerjaan" name="deceasedOccupation" value={form.deceasedOccupation} onChange={handleChange} />
        <Input label="Alamat" name="deceasedAddress" value={form.deceasedAddress} onChange={handleChange} />
        <Input label="Tanggal Meninggal" name="deathDate" type="date" value={form.deathDate} onChange={handleChange} />
        <Input label="Tempat Meninggal" name="deathPlace" value={form.deathPlace} onChange={handleChange} />
        <div className="md:col-span-2 font-semibold mt-4 mb-2">B. Data Pasangan</div>
        <Input label="Nama Lengkap" name="spouseName" value={form.spouseName} onChange={handleChange} />
        <Input label="Alias" name="spouseAlias" value={form.spouseAlias} onChange={handleChange} />
        <Input label="Bin/Binti" name="spouseBinBinti" value={form.spouseBinBinti} onChange={handleChange} />
        <Input label="NIK" name="spouseNik" value={form.spouseNik} onChange={handleChange} />
        <Input label="Tempat Lahir" name="spouseBirthPlace" value={form.spouseBirthPlace} onChange={handleChange} />
        <Input label="Tanggal Lahir" name="spouseBirthDate" type="date" value={form.spouseBirthDate} onChange={handleChange} />
        <Input label="Kewarganegaraan" name="spouseNationality" value={form.spouseNationality} onChange={handleChange} />
        <Input label="Agama" name="spouseReligion" value={form.spouseReligion} onChange={handleChange} />
        <Input label="Pekerjaan" name="spouseOccupation" value={form.spouseOccupation} onChange={handleChange} />
