...
import { CustomField, Letter, Resident, ResidentCustomField } from '../../types';
...
const ViewResident: React.FC = () => {
  ...
  const [customFieldValues, setCustomFieldValues] = useState<ResidentCustomField[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const residentLetters = await letterService.getLettersByResident(residentId);
  ...
  const getCustomFieldValue = (fieldId: number) => {
    const fieldValue = customFieldValues.find(fv => fv.customFieldId === fieldId);
    ...
    <div className="space-y-6">
      ...
      <Card title="Informasi Dasar">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <InfoItem label="Nama" value={resident.name} />
          <InfoItem label="Tanggal Lahir" value={formatDate(resident.birthDate)} />
          ...
          <InfoItem label="Alamat" value={resident.address} className="md:col-span-2" />
        </div>
      </Card>
      ...
      <Card title="Daftar Surat">
        {letters.length > 0 ? (
          ...
          <p className="text-gray-500 py-4 text-center">Belum ada surat terkait</p>
        )}
        ...
        <div className="mt-4">
          <Button
            variant="primary"
            fullWidth
            onClick={() => navigate('/letters/create', { state: { residentId: resident.id } })}
          >
            Buat Surat Baru
          </Button>
        </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-700">
            Apakah Anda yakin ingin menghapus data warga <strong>{resident.name}</strong>?
            {letters.length > 0 && (
              <span className="block mt-2 text-red-600">
                Warga ini memiliki {letters.length} surat terkait. Anda tidak dapat menghapus data warga ini.
              </span>
            )}
          </p>
