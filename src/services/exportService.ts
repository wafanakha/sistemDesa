...
export const exportService = {
  ...
  exportDatabase: async () => {
    ...
    saveAs(
      blob,
      `backup-administrasi-desa-${new Date().toISOString().slice(0, 10)}.json`
    );
  },
  ...
  exportLetterToPdf: async (
    ...
    try {
      ...
      doc.text(`Nomor: ${letter.letterNumber}`, 105, currentY + 10, {
        align: "center",
      });
      ...
      currentY += 10;
      const dateString = `${villageInfo.name}, ${formatDate(
        letter.issuedDate
      )}`;
...
async function addLetterheadToPdf(doc: jsPDF, villageInfo: VillageInfo) {
  ...
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text("PEMERINTAH " + villageInfo.regencyName.toUpperCase(), 105, 15, {
    align: "center",
  });
  doc.text("KECAMATAN " + villageInfo.districtName.toUpperCase(), 105, 22, {
    align: "center",
  });
  doc.setFontSize(18);
  doc.text("DESA " + villageInfo.name.toUpperCase(), 105, 30, {
    align: "center",
  });
  ...
  const websiteText = villageInfo.website
    ? `Website: ${villageInfo.website}`
    : "";

  doc.text(addressText, 105, 38, { align: "center" });
  doc.text(contactText + (websiteText ? ` - ${websiteText}` : ""), 105, 43, {
    align: "center",
  });
...
function createLetterHeader(villageInfo: VillageInfo): Paragraph[] {
  ...
  const headerParagraphs = [
    ...
    new Paragraph({
      text: `Telp: ${villageInfo.phoneNumber}${
        villageInfo.website ? ` - Website: ${villageInfo.website}` : ""
      }`,
      alignment: AlignmentType.CENTER,
    }),
  ];
